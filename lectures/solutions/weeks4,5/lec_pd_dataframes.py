...
aser = pd.Series(data=a,index=indexss)
bser = pd.Series(data=b,index=indexss)

df  = pd.DataFrame({'a':aser, 'b':bser})
print(df)
...
#impalce changes the original df, if have no inpalce you need a new variable
